...
3. `編集` を押して作成ページへ戻る
4. トップの `メール・LINEで署名` を押し、一覧から契約を選んで送信準備ページへ移動する
...
2. トップの `メール・LINEで署名` を押し、一覧から `メール・LINEで送る` を押す
3. `確認URL生成` を押す
...
8. 4つの確認項目にチェックし、タブレットまたはマウスで署名して `同意して電子署名を完了` を押す
9. `署名済み契約書を表示` を押し、帳票右上欄に署名画像・署名者名・署名日時が反映されるか確認する
10. `印刷・PDF保存` からA4で保存または印刷できるか確認する
...
- 元の契約が `完了` 状態になる
- 契約一覧の完了契約から `署名済みPDF` を再表示できる
- 署名済み帳票で入力内容が編集できず、印刷・PDF保存のみできる
...
1. 契約作成ページで必要項目を入力し、`クラウド保存` を押す
2. トップの `対面電子署名` を押す
3. 一覧から対象契約の `この契約に署名` を押す
4. お客様と契約内容を確認し、4つの確認項目にチェックする
5. タブレットに手書き署名し、`同意して電子署名を完了` を押す
6. `署名済み契約書を表示` を押す

チェック項目:

- 見積書は署名対象の一覧に出ない
- 選んだ契約がパスコード入力なしで対面署名画面に表示される
- 指やタッチペンで署名でき、書き直しもできる
- 署名が帳票右上の署名欄に反映される
- Supabase使用時は署名・同意項目・完了日時が保存され、元契約が `完了` になる
- 完了済み契約に再署名ボタンが出ない
