...
車両販売契約の入力内容をPDFに転記し、紙での手書き署名、メール・LINEでの遠隔電子署名、タブレットでの対面電子署名の3方式で完了できる契約作成システムです。
...
- `contract-list.html`: 契約一覧画面、遠隔署名・対面署名の契約選択画面
- `contract-contact.html`: メール・LINE契約の送信用文面作成画面
- `sales-consent.html`: お客様の遠隔署名画面、または店頭タブレットの対面署名画面
...
- `sales-sheet.js`: 帳票型ページの保存、上書き、自動計算、署名済み帳票の生成

契約書はA4縦の2ページ構成です。1ページ目が契約内容、2ページ目がオーダーオート特約事項で、両面印刷時は「長辺とじ」を使用します。見積書は特約事項を付けず、従来どおり1ページで出力します。
- `contract-contact.js`: メール・LINE送信用文面の作成、コピー、起動
- `sales-consent.js`: 確認URLの復号、重要事項確認、電子署名、署名済み帳票・完了メール作成
